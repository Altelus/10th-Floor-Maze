...
//---------------------------------- Utilities --------------------------------
//
// The Utilities Component supports the windowing system
//
...
char* strcpyCharToChar(char* dest, const char* src, int sizeDest) {
    *(dest + sizeDest) = '\0';
    while (sizeDest-- && (*dest++ = char(*src++))) ;
    return dest;
}
...
int sprintfAKV(wchar_t* str, int a,  const wchar_t* suffix) {

    wchar_t s[11];

    strcat(str, L" ", 200);
    strcat(str, itowc(s, a), 200);
    strcat(str, suffix, 200);
