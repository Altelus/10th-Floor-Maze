/* UserDialog Implementation - UserDialog Component - Windowing Branch
 *
 * UserDialog.cpp
 * fwk4gps version 1.0
 * gam666/dps901
...
#include "UserDialog.h"     // for UserDialog class declaration
...
//-------------------------------- UserDialog ---------------------------------
//
// The UserDialog Component collects the configuration information from the
// user
//
...
void UserDialog::populateSoundFileList(void* hwndw) {
    ...
    for (int msnd = 0; msnd < noSounds; msnd++) {
		...
		wchar_t availableFile[MAX_DESC + 1];
...
void UserDialog::drawBackgroundImage(void* hwnd, void* lpPaint) {
	
}
// dlgProc handles each message "msg" to the Dialog Box window "hwnd"
//

void UserDialog::onEraseBkgnd(void* hWnd) {
	//CBrush m_brush;
 //   CRect rect;    
	//GetClientRect(&rect);    
...
void UserDialog::onPaint(void* hWnd) {
	wchar_t* bg_filename = L"..\\..\\textures\\StartUpBackground.bmp";
	BITMAP bm;
	PAINTSTRUCT ps;
	HDC hDC = BeginPaint((HWND)hWnd, &ps);
	// Draw image here.
	HDC memoryDC = CreateCompatibleDC(hDC);
	HBITMAP hBitmap = (HBITMAP)LoadImage(NULL,bg_filename, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
