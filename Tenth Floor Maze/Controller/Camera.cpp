...
#include "Camera.h"          // for Camera class declaration
...
//
iViewing* Camera::coordinator = NULL;
...
//
//iCamera* CreateCameraWithMap(iContext* c, int map[20][20], int tileSize) {
//	return new Camera(c, map, tileSize);
//}

// constructor adds the Camera Instance to the Viewing Coordinator
//
//Camera::Camera(iContext* c, int map[20][20], int tileSize) : context(c), host(NULL) {
//	bumping_effect_coordinate = 0.0f;
//	breathingEffectCoordinate = 0.0;
//	breathing_effect_frequency = RYAN_DEFAULT_BREATHING_FREQUENCY;
//	breathing_effect_magnitude = RYAN_DEFAULT_breathing_effect_magnitude;
...
//}
Camera::Camera(iContext* c) : context(c), host(NULL) {
	//bumping_effect_coordinate = 0.0f;
	//breathingEffectCoordinate = 0.0;
	//breathing_effect_frequency = RYAN_DEFAULT_BREATHING_FREQUENCY;
	//breathing_effect_magnitude = RYAN_DEFAULT_breathing_effect_magnitude;
...
//
void Camera::attach(const iFrame* f) {
...
void Camera::update(int delta) {
 ...
 //   int z = context->get(GF_MS_ORIZ);
	...
	//	
	//// keyboard input
 //   if (context->pressed(TENTH_FLOOR_PLAYER_ADVANCE)) dz += delta * RYAN_CAMERA_ADVANCE_COEFFICIENT; /* RYAN: Added constant. Makes the movement faster */
 //   if (context->pressed(TENTH_FLOOR_PLAYER_RETREAT)) dz -= delta * RYAN_CAMERA_RETREAT_COEFFICIENT; /* RYAN: Added constant. See ModelSettings.h */
 //   if (context->pressed(TENTH_FLOOR_PLAYER_TURN_LEFT)) dy += delta * RYAN_KEYBOARD_PANNING_SENSITIVITY; /* RYAN: not needed */
 //   if (context->pressed(TENTH_FLOOR_PLAYER_TURN_RIGHT)) dy -= delta * RYAN_KEYBOARD_PANNING_SENSITIVITY; /* RYAN: not needed */
 ...
 //   }
	...
	//	breathing_effect_magnitude -= RYAN_breathing_effect_magnitude_COEFFICIENT;
	...
	//	if (breathing_effect_magnitude < RYAN_DEFAULT_breathing_effect_magnitude) breathing_effect_magnitude = RYAN_DEFAULT_breathing_effect_magnitude;
	//} else {
	//	breathing_effect_frequency += (RYAN_BREATHING_FREQUENCY_COEFFICIENT * 2);
	//	breathing_effect_magnitude += (RYAN_breathing_effect_magnitude_COEFFICIENT * 2);
	...
	//	if (breathing_effect_magnitude > RYAN_MAXIMUM_breathing_effect_magnitude) breathing_effect_magnitude = RYAN_MAXIMUM_breathing_effect_magnitude;
	//}
	//breathingEffectCoordinate += breathing_effect_frequency;
	////translate(0, sinf(breathingEffectCoordinate) * breathing_effect_magnitude, 0);
	//disp_y += sinf(breathingEffectCoordinate) * breathing_effect_magnitude;
