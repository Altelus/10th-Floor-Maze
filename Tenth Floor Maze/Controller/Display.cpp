/* Display Implementation - GraphicsCard Component - Direct Device Branch
 *
 * Display.cpp
 * fwk4gps version 1.0
 * gam666/dps901
...
#include "Display.h"         // for the Display class declaration
#include "DeviceLight.h"     // for access to DeviceLight::connectsThrough()
#include "Graphic.h"         // for access to Graphic::connectsThrough()
#include "DeviceTexture.h"   // for access to DeviceTexture::connectsThrough()
#include "Font.h"            // for access to Font::connectsThrough()
#include "Environment.h"     //GMOK: for access to Environment::connectsThrough()
...
// Component
//
// CreateDisplay creates the Display Object, which implements the Display Portal
//
...
// constructor initializes the instance variables and aligns the Display Portal
// with the other Elements of the Graphics Card Component
//
Display::Display(iContext* c) : context(c), hwnd(NULL), width_(0), height_(0), 
 lightIndex(NULL) {
	...
	// the Display Portal provides the API connectivity for the following Elements
	DeviceLight::connectsThrough(this);
	Graphic::connectsThrough(this);
	DeviceTexture::connectsThrough(this);
	Font::connectsThrough(this);
	Environment::connectsThrough(this);
}
...
// setConfiguration sets the configuration to be used for setup
//
void Display::setConfiguration() {
    ...
    context->set(GF_DS_WDTH, width_);
    context->set(GF_DS_HGHT, height_);
}
...
void Display::setupProjection() {
	...
	Matrix projection = ::projection(FIELD_OF_VIEW, aspect, NEAR_CLIPPING, 
     FAR_CLIPPING);
...
// getLightIndex gets the next light index on the display device
//
int Display::getLightIndex() {
...
bool Display::restore() {
	...
	if (rc) {
		...
		if (spriteManager_) spriteManager_->OnResetDevice();
    }
...
// alphaBlendOn turns on alphaBlending
//
void Display::alphaBlendOn() {

    // turn on alpha blending
    if (d3dd) d3dd->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE);
}

// alphaBlendOff turns off alphaBlending
//
void Display::alphaBlendOff() {

    // turn off alpha blending
    if (d3dd) d3dd->SetRenderState(D3DRS_ALPHABLENDENABLE, FALSE);
}

// beginSuperimpose prepares the display device for drawing of the HUD
//
...
// draw draws a single frame of the scene and superimposes the hud
//
...
// device returns the address of the display device itself
//
void* Display::device() const {

    return d3dd;
}

// spriteManager returns the address of the sprite manager
//
void* Display::spriteManager() const {
...
void Display::toggleFog(bool fe)
{
	d3dd->SetRenderState(D3DRS_FOGENABLE,fe);
}

//GMOK: sets the fog color
void Display::setFogColor(unsigned long c)
{
	if (fogEnabled)
		d3dd->SetRenderState(D3DRS_FOGCOLOR,c);
}

//GMOK: sets the fog start and end distance from player (fog thickness/density)
void Display::setFogStartEnd(float s, float e)
{
	if (fogEnabled)
	{
		d3dd->SetRenderState(D3DRS_FOGSTART,*(DWORD*)(&s));
		d3dd->SetRenderState(D3DRS_FOGEND,*(DWORD*)(&e));
	}
}
